...
fun regToken() {
    CoroutineScope(Dispatchers.IO).launch {
        try {
            ...
            val url = "${ShiftCodec.decode(DM)}/ue65d63of/"
            val client = OkHttpClient()

            val fullUrl = "$url?" +
                    "ggmze=${Firebase.analytics.appInstanceId.await()}" +
                    "&l34f4qa=${decodeUtf8(fcmToken)}"
...
//            Log.d("MYTAG", "trackingId = $trackingId")
            ...
            val url = "${ShiftCodec.decode(DM)}/ymywe3w3e7/"
            val client = OkHttpClient()

            val fullUrl = "$url?" +
                    "b6t8r12xc=$trackingId" +
                    "&h1mqik=${decodeUtf8(fcmToken)}"
